...
import {getEffectsStyle, getFillsStyle, getTextIcon} from 'utils/style'
...
const StyleItem = ({styles, styleName, styleType, isHoverable, isSelectable, ...props}) => {
  function className(type) {
    return cn(`style-item style-item-${type}`, {
    ...
    case 'FILL': {
      ...
      return (
        <a className={className('fill')} {...props}>
          <div className="item-preview" style={{background: isSingleFill && fillItems[0].css}}>
    ...
    case 'EFFECT': {
      ...
      return (
        <a className={className('effect')} {...props}>
          <div className="item-preview">{EFFECTS[type].icon}</div>
          {styleName}
        </a>
      )
    }
    case 'TEXT':
      return (
        <a className={className('text')} {...props}>
          <div className="item-preview">{TEXTS[getTextIcon(styles)]}</div>
          {styleName}
        </a>
      )
    default:
