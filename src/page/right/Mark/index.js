...
import {formattedNumber, getCode, getEffectsStyle, getFillsStyle} from 'utils/style'
    ...
    this.setState({
      exportSettings: exportSettings
        ? exportSettings
            ...
            .filter(({id}) => (currentIndex === 0 ? currentExportIds.indexOf(id) > -1 : id === elementData.node.id))
        : [],
    })
  }
      ...
      >
        ...
        <div className="props-section props-basic">
          ...
          <div className="section-items">
            ...
            {node.opacity !== undefined && <CopiableInput isQuiet label={t('opacity')} value={toFixed(node.opacity)} />}
            {node.cornerRadius && (
            ...
            {closestComponent.variantProperties && (
              <ul className="component-variants">
                {closestComponent.variantProperties.map((p, i) => (
                  <li key={i}>
                    <WithCopy text={Array.isArray(p) ? `${p[0]} = ${p[1]}` : p}>
                      {Array.isArray(p) ? (
                        <>
                          <span className="property-label">{p[0]}</span>
                          {p[1]}
                        </>
                      ) : (
                        p
              ...
              <>
                ...
                <InputGroup>
                  <CopiableInput
                    ...
                    value={!!node.strokeWeight ? formattedNumber(node.strokeWeight, globalSettings) : 'Mixed'}
                  />
                  {node.strokeDashes && (
                    <CopiableInput
                      label={t('stroke dash')}
                ...
                {!node.strokeWeight && (
                  <InputGroup className="mixed-strokes">
                    <CopiableInput
                      label={t('stroke top')}
                      value={formattedNumber(node.strokeTopWeight, globalSettings)}
                    />
                    <CopiableInput
                      label={t('stroke right')}
                      value={formattedNumber(node.strokeRightWeight, globalSettings)}
                    />
                    <CopiableInput
                      label={t('stroke bottom')}
                      value={formattedNumber(node.strokeBottomWeight, globalSettings)}
                    />
                    <CopiableInput
                      label={t('stroke left')}
                      value={formattedNumber(node.strokeLeftWeight, globalSettings)}
                    />
                  </InputGroup>
                )}
              </>
          ...
          <div className="props-section props-export">
            ...
            <ul className="section-items">
              {exportSettings.map((exportSetting, index) => (
                <li key={index}>
                  <ExportItem mode={mode} isMock={isMock} exportSetting={exportSetting} index={exportSetting.index} />
                </li>
              ))}
            </ul>
          </div>
