...
import {ChevronDown, FileText, Folder, Sidebar} from 'react-feather'
import {getAllFrames} from 'utils/frame'
import {getImageUrl} from 'utils/helper'

import './nestedFrames.scss'

const IconIndicator = ({id, level}) => {
...
const FrameItem = ({item, level, onSelect, selectedId}) => {
  ...
  return (
    <li key={item.id} className={cn('frame-item', {'frame-item-page': level === 0})}>
      ...
      >
        ...
        {item.children && (
          <div
            className={cn('item-chevron', {
              'item-chevron-collapsed': collapsed,
            })}
      ...
      {item.children && (
        <ul
          className={cn('item-children', {
            'item-children-collapsed': collapsed,
          })}
        >
          {item.children.map(item => (
            <FrameItem key={item.id} item={item} level={level + 1} onSelect={onSelect} selectedId={selectedId} />
          ))}
        </ul>
      )}
    </li>
...
const NestedFrames = ({visible, pagedFrames, mode, isMock, onSelect}) => {
  const [selectedId, setSelectedId] = useState('')

  const allFrames = useMemo(() => getAllFrames(pagedFrames), [pagedFrames])
