...
class Frames extends React.Component {
  ...
  changeFrameByIdProperty = prevProps => {
    ...
    if (currentFrameId && currentFrameId !== prevProps.globalSettings.currentFrameId && currentFrame) {
        ...
        <ul className={cn('list-container frames', {hide: !visible})}>
          {Object.keys(pagedFrames).map(key => {
            ...
            return (
              <li
                key={key}
                className={cn('frames-page', {
                  'frames-page-collapsed': collapsedPages.includes(key),
                })}
              >
