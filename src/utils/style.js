...
export const getRGBAObj = color => {
  ...
  return {
    r: (color.r * 255).toFixed(),
    g: (color.g * 255).toFixed(),
    b: (color.b * 255).toFixed(),
    alpha: toFixed(alpha),
  }
}

export const getColor = color => Color(getRGBAObj(color))
...
export const getCSSHEX = color => getColor(color).hex()

export const getHEXAlpha = alpha => ('0' + Number.parseInt(alpha * 255, 10).toString(16)).slice(-2).toUpperCase()
...
export const getCSSHSLA = (color, opacity) => {
  ...
  const hsl = c
    .split(',')
    .map((part, index) => (index === 0 ? part.split('(')[0] + '(' + toFixed(part.split('(')[1] - 0) : part))
    .join()
  return hsl
}
...
export const getOpacity = opacity => toFixed(opacity) || 1
    ...
    finalLineHeight =
      lineHeightUnit === 'PIXELS'
        ...
        : lineHeightUnit === 'INTRINSIC_%'
          ? 'normal'
          : toFixed((lineHeightPercentFontSize || 100) / 100)
  } else {
    // from plugin
    finalLineHeight =
      lineHeightUnit === 'PIXELS'
        ...
        : lineHeightUnit === 'AUTO'
          ? 'normal'
          : toFixed((lineHeight || 100) / 100)
  }
  return finalLineHeight
}

export const getTextStyle = (textItems, isInPropPanel) => {
  const {fontFamily, fontWeight, fontSize, textDecoration, letterSpacing, lineHeightUnit, textAlignHorizontal} =
    textItems
...
export const getCode = (node, fillItems, strokeItems, effectItems, textStyle, globalSettings) => {
  ...
  // font style
  if (node.type === 'TEXT') {
    const {fontFamily, fontWeight, fontSize, lineHeightUnit, lineHeight, letterSpacing, textAlign, textDecoration} =
      textStyle
