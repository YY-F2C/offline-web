...
export const isAllImageFill = fills => fills.filter(fill => fill.type === 'IMAGE').length === fills.length
...
export function getImageUrl(item, mode, isMock) {
  ...
  return mode === 'local' || isMock ? `${process.env.PUBLIC_URL}/data/${fileName}` : item.image.url
}
